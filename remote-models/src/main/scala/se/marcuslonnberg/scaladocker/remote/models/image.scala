...
case class ImageName(registry: Option[String] = None, namespace: Option[String] = None,
  ...
  require(repository.matches("[a-z0-9-_.]+"), s"Repository name ('$repository') can only contain characters [a-z0-9-_.]")
  require(tag.matches("[A-Za-z0-9_.-]{2,30}"), s"Tag name ('$tag') can only contain characters [a-z0-9_] and have a size between 2 and 30")
